...
def scan(
    ...
    try:
        ...
        if preview:
            ...
            # NEW: Save blank template images for use during extraction
            console.print("  [dim]Saving blank template images...[/dim]")
            import shutil
            blank_images = {}
            for i, img_path in enumerate(image_paths, start=1):
                blank_filename = f"{form_id}_blank_page_{i}.png"
                blank_dest = output_path / blank_filename
                ...
                try:
                    ...
                    with Image.open(img_path) as img:
                        if img.mode in ('RGBA', 'P'):
                            img = img.convert('RGB')
                        img.save(blank_dest, 'PNG')
                except ImportError:
                    shutil.copy(img_path, blank_dest)
                
                blank_images[f"page_{i}"] = blank_filename
                console.print(f"  [dim]Saved: {blank_filename}[/dim]")
            
            # Add blank images info to form schema
            form_schema.blank_images = blank_images
            ...
            # Save complete form schema (now includes blank_images)
...
def extract(
    ...
    try:
        ...
        if schema:
            ...
            # Check for blank template images
            if form_schema.blank_images:
                blank_image_paths = []
                for i in range(1, form_schema.total_pages + 1):
                    ...
                    if blank_filename:
                        ...
                        if blank_path.exists():
                            blank_image_paths.append(blank_path)
                        else:
                ...
                if valid_blanks > 0:
                    console.print(f"[cyan]Found {valid_blanks} blank template images for comparison[/cyan]")
        ...
        # Adjust blank image paths if page range was specified
        if blank_image_paths and (start_page > 1 or end_page is not None):
        ...
        # Extract form (with blank templates if available)
